...
{
    public class CopyPackageFilesResponseTests
    {
        [Fact]
        public void Constructor_ThrowsForUndefinedResponseCode()
        {
            var exception = Assert.Throws<ArgumentException>(
                () => new CopyPackageFilesResponse((MessageResponseCode)int.MaxValue, new[] { "a" }));
        ...
        {
            var exception = Assert.Throws<ArgumentException>(
                () => new CopyPackageFilesResponse(MessageResponseCode.Success, copiedFiles: null));
        ...
        {
            var exception = Assert.Throws<ArgumentException>(
                () => new CopyPackageFilesResponse(MessageResponseCode.Success, new string[] { }));
        ...
        {
            var response = new CopyPackageFilesResponse(MessageResponseCode.Success, new [] { "a" });
        ...
        public void JsonDeserialization_ReturnsCorrectObjectForSuccess()
        {
            ...
            var response = JsonSerializationUtilities.Deserialize<CopyPackageFilesResponse>(json);
        ...
        public void JsonDeserialization_ThrowsForInvalidResponseCode(string json)
        {
            Assert.Throws<JsonSerializationException>(
                () => JsonSerializationUtilities.Deserialize<CopyPackageFilesResponse>(json));
        }
